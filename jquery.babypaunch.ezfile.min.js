"use strict";var ezfile={file:function($element){var name=$element[0].files[0].name;return{name:name.substr(0,name.lastIndexOf(".")),ext:name.substr(name.lastIndexOf(".")+1)}},isExt:function($element,data){if(data.ext===undefined){return!0}
var result=!0;var name=this.file($element);switch($.type(data.ext)){case "string":if(data.ext.toLowerCase()!==name.ext){result=!1;alert("업로드 파일의 확장자는 대소문자 구분없이 "+data.ext+"만 허용됩니다.")}
break;case "array":result=!1;for(var i=0;i<data.ext.length;i++){if(data.ext[i].toLowerCase()===name.ext){result=!0;break}}
if(!result){alert("업로드 파일의 확장자는 대소문자 구분없이 "+data.ext.join()+"만 허용됩니다.")}
break;default:result=!1;alert("파일의 확장자는 문자열이나 배열만 입력할 수 있습니다.");break}
return result},uploadable:function($element,data,callback){var result=!0;var fr=new FileReader();fr.readAsDataURL($element[0].files[0]);fr.onload=function(){if(data.byte!==undefined){var flag=data.byte.toLowerCase();var bytes=Number(data.byte.replace(/[^0-9]/g,""));for(var i=0,arr=["k","m","g","t","p"];i<arr.length;i++){if(flag.lastIndexOf(arr[i])!==-1){result=$element[0].files[0].size<bytes*Math.pow(1024,i+1)?!0:!1}}
if(!result){alert("업로드 파일의 용량은 "+data.byte+"이하로 제한합니다.");callback(result);return}}
